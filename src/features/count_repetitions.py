import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from DataTransformation import LowPassFilter
from scipy.signal import argrelextrema
from sklearn.metrics import mean_absolute_error

pd.options.mode.chained_assignment = None


# Plot settings
plt.style.use("fivethirtyeight")
plt.rcParams["figure.figsize"] = (20, 5)
plt.rcParams["figure.dpi"] = 100
plt.rcParams["lines.linewidth"] = 2


# Load Data and Recalculate Squares
df = pd.read_pickle("../../data/interim/data_processed.pkl")
df = df[df["label"] != "rest"]

acc_r = df["acc_x"] ** 2 + df["acc_y"] ** 2 + df["acc_z"] ** 2 
gyr_r = df["gyr_x"] ** 2 + df["gyr_y"] ** 2 + df["gyr_z"] ** 2 

df["acc_r"] = np.sqrt(acc_r)
df["gyr_r"] = np.sqrt(gyr_r)

# Split Data Based on Label
bench_df = df[df["label"] == 'bench']
squat_df = df[df["label"] == 'squat']
ohp_df = df[df["label"] == 'ohp']
dead_df = df[df["label"] == 'dead']
row_df = df[df["label"] == 'row']

# Identify Patterns by Plotting
plot_df = bench_df.copy()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["acc_x"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["acc_y"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["acc_z"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["acc_r"].plot()

plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["gyr_x"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["gyr_y"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["gyr_z"].plot()
plot_df[plot_df["set"] == plot_df["set"].unique()[0]]["gyr_r"].plot()


# LowPass Config
sampling_frequency = 1000 / 200

LowPass = LowPassFilter()

# Applying LowPass and Identifying Ideal Cutoff Frequencies
bench_set = bench_df[bench_df["set"] == bench_df["set"].unique()[0]]
squat_set = squat_df[squat_df["set"] == squat_df["set"].unique()[0]]
ohp_set = ohp_df[ohp_df["set"] == ohp_df["set"].unique()[0]]
dead_set = dead_df[dead_df["set"] == dead_df["set"].unique()[0]]
row_set = row_df[row_df["set"] == row_df["set"].unique()[0]]


squat_set["acc_r"].plot()

col = "acc_y"
LowPass.low_pass_filter(bench_set, col, sampling_frequency=sampling_frequency, cutoff_frequency=0.4, order=10)[col + "_lowpass"].plot()


# Function to Calculate Completed Reps
col = "acc_y"
def count_reps(dataset, cutoff=0.4, order=10, col="acc_r"):
    data = LowPass.low_pass_filter(dataset, col, sampling_frequency=sampling_frequency, cutoff_frequency=cutoff, order=order)[col + "_lowpass"]

    indeces = argrelextrema(data.values, np.greater)
    peaks = data.iloc[indeces]
    
    return len(peaks)

count_reps(bench_set, cutoff=0.4)
count_reps(squat_set, cutoff=.35)
count_reps(ohp_set, cutoff=0.35)
count_reps(dead_set, cutoff=0.4)
count_reps(row_set, cutoff=0.63)


# Create DataFrame to Test Accuracy
df["reps"] = df["category"].apply(lambda x: 5 if x=="heavy" else 10)
rep_df = df.groupby(["label", "category", "set"])["reps"].max().reset_index()

rep_df["predicted_reps"] = 0

for s in df["set"].unique():
    subset = df[df["set"] == s]
    
    col = "acc_r"
    cutoff = 0.4
        
    if (subset["label"].iloc[0] == "ohp"):
        cutoff = 0.35
        
    if (subset["label"].iloc[0] == "row"):
        cutoff = 0.63
    
    reps = count_reps(subset, cutoff=cutoff, col=col)
    
    rep_df.loc[rep_df["set"] == s, "predicted_reps"] = reps
        
        
# Evaluation of Results
error = mean_absolute_error(rep_df["reps"], rep_df["predicted_reps"])